...
function parseLine(line: string): KeywordData | null {
    ...
    const parseArray = (str: string): string[] => {
        if (!str || str === '') return [];
        return unescapeFromTable(str).split(';').map(s => s.trim()).filter(s => s);
    };
...
async function crawlCompetitorContent(urls: string[]): Promise<{ headings: string, meta: string }> {
    ...
    for (const url of urls) {
        ...
        try {
            process.stdout.write(`${colors.dim}    - Crawling (${results.length + 1}/${MAX_SUCCESSES}): ${url.substring(0, 50)}...${colors.reset}`);
            ...
            if (headings.length === 0) {
                process.stdout.write(`\r${colors.yellow}    ⚠️  Skipped: No headings found at ${url.substring(0, 40)}...    \n${colors.reset}`);
                continue;
            }
            ...
            process.stdout.write(`\r${colors.green}    ✅ Success (${results.length}/${MAX_SUCCESSES}): ${url.substring(0, 50)}...    \n${colors.reset}`);
        } catch (e) {
            process.stdout.write(`\r${colors.red}    ❌ Failed: ${url.substring(0, 50)}... (${e instanceof Error ? e.message : 'Error'})    \n${colors.reset}`);
        }
    }
...
async function main() {
    ...
    console.log(`${colors.dim}Using Adapter: ${colors.reset}${colors.bright}${adapter.providerName}${colors.reset}\n`);
    ...
    for (const item of selectedToUpdate) {
        ...
        try {
            ...
            if (metrics) {
                ...
                if (kw.includes('how') || kw.includes('tutorial') || kw.includes('guia') || kw.includes('guide')) {
                    data.intent = 'Informational';
                } else if (kw.includes('best') || kw.includes('top') || kw.includes('vs') || kw.includes('mejor') || kw.includes('comparativa')) {
                    data.intent = 'Commercial';
                } else if (kw.includes('comprar') || kw.includes('precio') || kw.includes('price') || kw.includes('buy') || kw.includes('service')) {
                ...
                const formula2 = data.keyword.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                ...
                console.log(`    ${colors.green}✅ Success: Vol ${data.volume}, Diff ${data.difficulty}${colors.reset}\n`);
            }
        } catch (e) {
            console.error(`    ${colors.red}❌ Error: ${e instanceof Error ? e.message : String(e)}${colors.reset}\n`);
        }
    ...
    for (const line of lines) {
        ...
        if (line.includes('| Keyword') && line.trim().startsWith('|')) {
            updatedLines.push('| Keyword | Target URL | Volume | Difficulty | Intent | Status | Last Updated | Source | Related Searches | PAA Count | Top Domain | Has AI Overview | SERP Features | Competitor Headings | Competitor Meta | Opportunity Score | Recommended Format | Cluster Type | Suggested Anchor Text |');
    ...
    console.log(`${colors.green}${colors.bright}✨ Finished! File updated at ${KEYWORDS_FILE_PATH}${colors.reset}\n`);
